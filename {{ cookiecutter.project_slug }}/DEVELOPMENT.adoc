...
To run the tests, simply run `npm run test` on the command line.
...
[TIP]
Packaging is run on each Contribution by a GitHub Workflow,
which automagically commits the generated files back to the repository.
